...
class TestEnvironmentBenchmark:
    def test_run_with_mock_env(self):
        class MockEnv:
            ...
            def reset(self):
                self._step_count = 0
                obs = {p: {"tensor": np.zeros((2504,)), "action_mask": np.ones(81, dtype=bool)} for p in self.possible_agents}
                return obs, {}
        ...
        class TestModel:
            def initial_inference(self, batch_tensor):
                ...
                return {
                    ...
                    'policy_logits': torch.zeros(batch_size, 81, device=device),
    ...
    async def test_get_actions_records_timing(self):
        ...
        observations = {
            f"player_{i}": {
                ...
                "action_mask": np.ones(81, dtype=bool)
            }
            for i in range(8)
        }
